...
**性能说明：** GSC / GA4 同步不会在普通前台访问中直接请求 Google；手动同步只在后台 AJAX 中运行，自动同步通过 WP-Cron 每天执行一次，结果以 `autoload=false` 的 option 缓存。若站点流量很大或希望完全关闭自动数据同步，可使用过滤器：

```php
add_filter( 'gml_seo_analytics_auto_sync_enabled', '__return_false' );
```
...
**模块结构：** 多语言功能现在作为 GML AI SEO 的内部模块组织：PHP 代码位于 `includes/modules/translate/`，静态资源位于 `assets/translate/`。为了兼容已经迁移的站点，`GML_*` 类名、`gml_*` option、短代码和数据库表名继续保留，所以升级不会丢失翻译记忆库或语言配置。
...
1.9.7 — 完整更新日志见 [CHANGELOG.md](CHANGELOG.md)
